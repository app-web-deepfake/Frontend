...
export default function Upload({ onClose }) {
    ...
    const handleFileChange = (e) => {
        ...
        if (err) { setFile(null); setError(err); return; }
        setFile(f);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!consentAccepted)  { setError('Debes aceptar el tratamiento de datos para continuar'); return; }

        try {
            setUploading(true);

            // 1. URL presignada
            const { uploadUrl, fields, fileUrl } = await getPresignedUrl(file.name, file.type);

            // 2. Subir a S3
            await uploadToS3(uploadUrl, fields, file);

            // 3. Analizar — la respuesta YA contiene el resultado completo
            const analysisResponse = await startAnalysis(fileUrl, file.name);
            const analysisId = analysisResponse.analysisId;

            // 4. Navegar pasando el resultado completo en el state
            //    Así ResultPage no necesita hacer otra llamada al backend
            navigate(`/result/${analysisId}`, {
                state: {
                    fileUrl,
                    result: analysisResponse.result,   // ← resultado completo directo
                }
            });
            ...
            <div className="upload-modal">
                ...
                <div className="privacy-info">
                    ...
                    <ul className="privacy-list">
                        <li><strong>Uso:</strong> Tu archivo será analizado mediante Inteligencia Artificial para detectar posibles manipulaciones digitales.</li>
                        <li><strong>Servicio externo:</strong> El análisis se realiza a través de ZeroTrue, un servicio especializado en detección de contenido generado por IA.</li>
                        <li><strong>Almacenamiento temporal:</strong> Los archivos se guardan de forma segura en Amazon S3 durante 7 días y luego se eliminan automáticamente.</li>
                        <li><strong>Confidencialidad:</strong> No compartimos tu contenido con terceros ni lo utilizamos para otros fines.</li>
                    </ul>
                </div>
