...
export default function Historial() {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('¿Eliminar este análisis del historial?')) return;
        setDeletingId(id);
        try {
            await deleteAnalysis(id);
            setAnalisis(prev => prev.filter(a => a._id !== id));
        } catch (err) {
            alert(err.message);
        } finally {
            setDeletingId(null);
                ...
                {!loading && !error && analisis.length > 0 && (
                    <div className="historial-filtros">
                        ...
                        {analisis.length > 0 && (
                            ...
                            >
                                <IconTrash size={14} color="white" /> Borrar todo
                            </button>
                        )}
                    </div>
                )}
                        ...
                        {analisisFiltrado.map((item, i) => {
                            ...
                            return (
                                <div key={item._id || i} className="historial-card">
                                    ...
                                    >
                                        {deletingId === item._id
                                            ? <span className="btn-delete-spinner" />
